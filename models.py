...
class HyperNet_Text(nn.Module):
    ...
    def __init__(self, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size):
        super(HyperNet_Text, self).__init__()
    ...
    def forward(self, vec):
        ...
        # input features for hyper net
        hyper_in_feat = vec.view(-1, self.hyperInChn, feature_size, feature_size)
...
class HyperNet_All(nn.Module):
    ...
    """
    def __init__(self, lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size):
        super(HyperNet_All, self).__init__()
        ...
        # Hyper network part, conv for generating target fc weights, fc for generating target fc biases
        self.fc1w_conv = nn.Conv2d(self.hyperInChn, int(self.target_in_size * 2 * self.f1 / feature_size ** 2), 3,  padding=(1, 1))
        self.fc1b_fc = nn.Linear(self.hyperInChn, self.f1)
        ...
        self.fc5w_fc = nn.Linear(self.hyperInChn, self.f4 * 2)
        self.fc5b_fc = nn.Linear(self.hyperInChn, 2)
...
class HyperNet_All_2023(nn.Module):
    """
    Hyper network for learning perceptual rules.
    ...
    """
    def __init__(self, lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size):
        super(HyperNet_All_2023, self).__init__()
        ...
        self.fc5w_fc = nn.Linear(self.hyperInChn, self.f4 * 3)
        self.fc5b_fc = nn.Linear(self.hyperInChn, 3)
    ...
    def forward(self, img):
        ...
        target_fc5w = self.fc5w_fc(self.pool(hyper_in_feat).squeeze()).view(-1, 3, self.f4, 1, 1)
        target_fc5b = self.fc5b_fc(self.pool(hyper_in_feat).squeeze()).view(-1, 3)
