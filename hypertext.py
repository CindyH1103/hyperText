...
class HyperIQASolver_Text(object):
    ...
    def __init__(self, config, path, train_idx, test_idx):
        ...
        self.model_hyper = models.HyperNet_Text(12, 672, 128, 64, 32, 16, 8).cuda()
        self.model_hyper.train(True)

        self.l1_loss = torch.nn.L1Loss().cuda()

        self.hypernet_params = self.model_hyper.parameters()
        self.lr = config.lr
        ...
        train_loader = data_loader.DataLoader(config.dataset, path, train_idx, config.patch_size,
                                              config.train_patch_num, config.hyper_text, batch_size=config.batch_size, istrain=True)
        test_loader = data_loader.DataLoader(config.dataset, path, test_idx, config.patch_size, config.test_patch_num, config.hyper_text,
                                             istrain=False)
    ...
    def train(self):
        ...
        for t in range(self.epochs):
            ...
            for text, text_feature, label in self.train_data:
                text = (text.cuda()).clone().detach()
                ...
                paras = self.model_hyper(text)  # 'paras' contains the network weights conveyed to target network
                ...
                pred = model_target(text_feature)
    ...
    def test(self, data):
        ...
        for img, text_feature, label in data:
