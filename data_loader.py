...
class DataLoader(object):
    ...
    def __init__(self, dataset, path, img_indx, patch_size, patch_num, hyper_text, hyper_all, batch_size=1, istrain=True):
        ...
        if (dataset == 'live') | (dataset == 'csiq') | (dataset == 'tid2013') | (dataset == 'livec'):
            ...
            if istrain:
                ...
                transforms = torchvision.transforms.Compose([
                    ...
                    torchvision.transforms.Normalize(mean=(0.485, 0.456, 0.406),
                                                     std=(0.229, 0.224, 0.225))])
        elif dataset == 'bid' or dataset == "AGIQA-3k" or dataset == "AIGCIQA2023":
            if istrain:
                transforms = torchvision.transforms.Compose([
                    torchvision.transforms.RandomHorizontalFlip(),
        ...
        if dataset == 'live':
            ...
            if hyper_text:
                self.data = folders.AGIQA_3k_Text_Folder(
                    root=path, index=img_indx, transform=transforms, patch_num=patch_num)
            elif hyper_all:
                self.data = folders.AGIQA_3k_All_Folder(
                    root=path, index=img_indx, transform=transforms, patch_num=patch_num)
            else:
                self.data = folders.AGIQA_3k_Folder(
                    root=path, index=img_indx, transform=transforms, patch_num=patch_num)
        elif dataset == "AIGCIQA2023":
            if hyper_text:
                self.data = folders.AIGCIQA2023_Text_Folder(
                    root=path, index=img_indx, transform=transforms, patch_num=patch_num
                )
            elif hyper_all:
                self.data = folders.AIGCIQA2023_All_Folder(
                    root=path, index=img_indx, transform=transforms, patch_num=patch_num
                )
            else:
                self.data = folders.AIGCIQA2023_Folder(
                    root=path, index=img_indx, transform=transforms, patch_num=patch_num
                )
